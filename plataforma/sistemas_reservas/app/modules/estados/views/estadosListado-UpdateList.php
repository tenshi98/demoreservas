    ...
    <tbody>
        ...
        if(is_array($data['arrList'])&&!empty($data['arrList'])){
            ...
            foreach($data['arrList'] as $crud){
                //Variables
                $encryptedId = $data['Fnc_Codification']->encryptDecrypt('encrypt', $crud['idEstadoReserva']);
                $level       = $data['UserAccess']['LevelAccess'];
                $Entidad     = addslashes($crud['Nombre']); ?>
                <tr>
                    <td><?php echo $crud['Nombre']; ?></td>
                    <td><?php echo '<span class="badge-sp1 badge-sp1-bg-info" style="background-color: '.colorMasClaro($crud['Color'], 0.40).';color: '.$crud['Color'].'; border: 1px solid '.$crud['Color'].';">'.$crud['Nombre'].'</span>';?></td>
                    <td>
