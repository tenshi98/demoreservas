    ...
    <tbody>
        ...
        if(is_array($data['arrList'])&&!empty($data['arrList'])){
            ...
            foreach($data['arrList'] as $crud){
                //Variables
                $encryptedId = $data['Fnc_Codification']->encryptDecrypt('encrypt', $crud['idRecurso']);
                $level       = $data['UserAccess']['LevelAccess'];
                $Entidad     = addslashes($crud['Nombre']); ?>
                <tr>
                    <td><?php echo $crud['Nombre']; ?></td>
                    <td><?php echo $crud['TipoCobro']; ?></td>
                    <td><?php echo ($crud['Valor'] != 0 ? $data['Fnc_DataNumbers']->Valores($crud['Valor'], 0) : ''); ?></td>
                    <td>
