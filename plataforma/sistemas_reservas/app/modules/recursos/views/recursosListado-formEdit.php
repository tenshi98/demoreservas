...
<form id="FormEditData" name="FormEditData" autocomplete="off" method="POST" action="" role="form" novalidate enctype="multipart/form-data" aria-label="Formulario de ejecucion">
    ...
    <div class="modal-body">
        ...
        $data['Fnc_FormInputs']->formSelect([                 'Placeholder' => 'Tipo de Cobro', 'Name' => 'idTipoCobro',  'Id' => 'Edit_idTipoCobro',  'Value' => ($data['rowData']['idTipoCobro'] ?? ''),   'Required' => 2,'arrData' => $data['arrTipoCobro']]);
        $data['Fnc_FormInputs']->formInput(['FormType' => 5,  'Placeholder' => 'Valor',         'Name' => 'Valor',        'Id' => 'Edit_Valor',        'Value' => ($data['rowData']['Valor'] ?? ''),         'Required' => 1,'Icon' => 'bi bi-currency-dollar']);

        //datos ocultos
        $data['Fnc_FormInputs']->formInputHidden(['Name' => 'idRecurso','Value' => $data['rowData']['idRecurso'],'Required' => 2]);
        ?>
    </div>
...
<script>
    ...
    /**********************************************************************/
    //Se ocultan los divs
    document.getElementById('div_Edit_Valor').style.display  = 'none';
    //cargo
    document.getElementById("Edit_idTipoCobro").onchange = function() {cngFnc_Edit_idTipoCobro()}
    //al cargar
    $(document).ready(function(){cngFnc_Edit_idTipoCobro();});
    //Ejecutar logica
    function cngFnc_Edit_idTipoCobro() {
        //obtengo los valores
        let idTipoCobro = $("#Edit_idTipoCobro").val();
        //selecciono
        if (idTipoCobro != "") {
            //selecciono
            switch (idTipoCobro) {
                ...
                //Por Asistente
                case '3':
                    document.getElementById('div_Edit_Valor').style.display  = '';
                    break;
                //el resto
                default:
                    document.getElementById('div_Edit_Valor').style.display  = 'none';
